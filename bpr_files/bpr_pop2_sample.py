...
class BPR:
    ...
    def test(self, data):
        ...
        #pred = [i for i in self._predict_user(user_id) if i not in know_items_ids]
        pred = [i for i in self._predict_user(user_id)]
        return list(enumerate(pred))
        
    def fit(self, user_item_train_df):
        # self.writer = SummaryWriter(logdir="./runs/default")
        self.writer = SummaryWriter(logdir="./runs/new_sample_full-divkl")
        # self.writer = SummaryWriter(logdir="./runs/new_sample_full-10lr-1minus_10xlr-update1")
        ...
        for _ in loop:
            for _ in range(batch_iters):
                ...
                sampled = self._sample2(n_users, n_items, indices, indptr)
                sampled_users, sampled_pos_items, s2 = sampled
                self._update2(
                    sampled_users, sampled_pos_items, s2,
                    tradeoff=tradeoff,
                    distribution_column=distribution_column,
                    p_t_u_all_users=p_t_u_all_users, p_t_i_all_items=p_t_i_all_items,
                    dataset=dataset, movies_data=movies_data
    ...
    def _sample2(self, n_users, n_items, indices, indptr):
        """sample batches of random triplets u, i, j"""
        ...
        for user in sampled_users:
            ...
            pos_items = indices[indptr[user]:indptr[user + 1]]
            pos_item = np.random.choice(pos_items, 10)

            sampled_item = np.random.choice(
                n_items, size = 10, replace = False)
            sampled_items.append(sampled_item)

            sampled_pos.append(pos_item)


        return sampled_users, sampled_items, sampled_pos

    def _update2(self, u, i, iplus, tradeoff, distribution_column, p_t_u_all_users, p_t_i_all_items, dataset, movies_data):
        ...
        m = 0
        for uu, ii, iiplus in zip(u, i, iplus):
            preds = [
                (i_, index+1) for index, i_ in enumerate(ii)
            ]

            preds_plus = [
                (i_, index+1) for index, i_ in enumerate(iiplus)
            ]
            ...
            KL_uiplus = Metrics.get_user_KL_divergence(
                dataset, movies_data,
                user_id=uu, recomended_items=preds_plus,
                p_g_u=p_t_u_all_users[uu], distribution_column=distribution_column,
                p_t_u_all_users=p_t_u_all_users, 
                p_t_i_all_items=p_t_i_all_items
            )
            ...
            # r_uij = KL_ui/KL_uiplus#((1-KL_ui) - (1-KL_uiplus))/KL_void
            # sigmoid = r_uij#np.exp(-r_uij) / (1.0 + np.exp(-r_uij))
            # sigmoid = np.exp(-r_uij) / (1.0 + np.exp(-r_uij))
            # r_uij = 1-((1-KL_ui)/(1-KL_uiplus))
            r_uij = ((1-KL_ui/KL_void) - (1-KL_uiplus/KL_void)).astype(np.float128)
            sigmoid = r_uij
            # sigmoid = np.exp(-r_uij) / (1.0 + np.exp(-r_uij)).astype(np.float128)
            m+=sigmoid
            # print("KL", KL_ui, KL_void)
            # print("RR", r_uij)
            # print("sigmoid", sigmoid)
            if math.isnan(sigmoid) or math.isnan(r_uij) or math.isnan(KL_uiplus):
                print("Predictions: ", preds)
                input()
            user_u = self.user_factors[uu]
            # sigmoid_tiled = np.tile(sigmoid, (self.n_factors, 1)).T
            for (iii, _), (jjj, _) in zip(preds, preds_plus):
                itemsi = self.item_factors[iii]
                itemsj = self.item_factors[jjj]
                
                r_uij = np.sum(user_u * (itemsi - itemsj))

                

                rui = user_u @ itemsi.T #* (1-KL_ui)
                ruj = user_u @ itemsj.T #* (1-KL_uiplus)
                ...
                self.writer.add_scalar('DEBUG/ruij', r_uij, self.count4)
                self.writer.add_scalar('DEBUG/SIGMOID', sigmoid, self.count4)
                self.count4 += 1

                # print("RRRS", r_uij, sigmoid)
                r_uij2 = ((1-KL_ui) - (1-KL_uiplus))/KL_void
                
                grad_u = sigmoid * (itemsi - itemsj)  + self.reg * user_u
                grad_i = sigmoid * user_u  + self.reg * itemsi
                grad_j = sigmoid * -user_u  + self.reg * itemsj
                self.user_factors[u] += self.learning_rate * grad_u 
                self.item_factors[iii] += self.learning_rate * grad_i
                self.item_factors[jjj] += self.learning_rate * grad_j
        self.count += 1
        m = m/len(u)
    ...
    def _update(self, u, i, j, tradeoff, distribution_column, p_t_u_all_users, p_t_i_all_items, dataset, movies_data):
        ...
        for uu, ii, jj in zip(u, i, j):
            ...
            m += (1-(KL_ui/KL_void))
            if math.isnan(KL_ui) or math.isnan(KL_void):
                for aaa in preds:
                    if math.isnan(aaa[1]):
                        print("FACTOR NAN: ", self.item_factors.T[aaa[0]], self.user_factors[uu])
                print("Predictions: ", preds)
                print("KL",KL_ui, KL_void )
                input()
            # print("KL",KL_ui, KL_void )
        ...
        r_uij = np.sum(user_u * (item_i - item_j), axis = 1).astype(np.float128)
        sigmoid = np.exp(-r_uij) / (1.0 + np.exp(-r_uij))
        m2 = sigmoid.mean()
        self.writer.add_scalar('LOSS/update1', m2, self.count2)
