...
def run_experiment(model_name_list, model_list, dataset, df, calibration_column_list=["popularity"]):
    metrics_data = []
    all_genres = set()
    ...
    for ind in [int(indiceee)]:
        ...
        for model_name, model in zip(model_name_list, model_list):
            ...
            del model
            gc.collect()
            pd.DataFrame(metrics_data).to_csv(f"./results/ml-20m/popularity_tmp_{model_name}.csv")
...
if __name__ == '__main__':
    ...
    dataset.load_local_movielens_dataset("./datasets/ml-20m", type="20m", index=indiceee)
    ...
    if dataset is not None:
        ...
        df = run_experiment(models_names, models, dataset, df, calibration_column_list=['popularity'])
        ...
        df.to_csv(f"./results/ml-20m/popularity_{time.time()}_complete_index1.csv")