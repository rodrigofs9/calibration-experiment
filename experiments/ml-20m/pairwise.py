...
def run_experiment(dataset, df):
    ...
    for ind in [int(fold)]:
        ...
        exp_results = exp.map(f, list(islice(test["user"], 200)))
        exp.close()
        exp.join()
        ...
        gc.collect()
        pd.DataFrame(metrics_data).to_csv(f"./results/ml-20m/pairwise_tmp.csv")
...
if __name__ == '__main__':
    ...
    dataset = MLDataset()
    dataset.load_local_movielens_dataset("./datasets/ml-20m", type="20m-new", index=fold)
    ...
    if dataset is not None:
        ...
        df.to_csv(f"./results/ml-20m/pairwise_{time.time()}_complete_index1.csv")