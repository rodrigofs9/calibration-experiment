...
def run_experiment(model_name_list, model_list, dataset, df, calibration_column_list=["genres"]):
    ...
    for ind in [int(indiceee)]:
        ...
        for model_name, model in zip(model_name_list, model_list):
            ...
            #    dataset = dataset.sample(frac=0.05, random_state=1)
            f = partial(
            ...
            del model
            gc.collect()
            pd.DataFrame(metrics_data).to_csv(f"./results/yahoo_song/abdoula_tmp_{model_name}.csv")
...
if __name__ == '__main__':
    ...
    if dataset is not None:
        ...
        df.to_csv(f"./results/yahoo_song/abdoula_{time.time()}_complete_index1.csv")