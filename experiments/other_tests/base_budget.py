...
from source.metrics.metrics_calculation import calculate_tradeoff_metrics2
...
from source.rerank.rerank_algo import run_user5
...
# dataset.clean_dataset(users=100)
...
def run_experiment(model_name_list, model_list, dataset, df, calibration_column_list=["genres"]):
    ...
    movies_bins_profile,
            ...
            for calibration_column in calibration_column_list:
                ...
                joint_tradeoffs = []
                for ww in [0.25, 0.5, 1]:
                    for www in ["VAR", "GC", 0, 0.2, 0.4, 0.6, 0.8, 1]:
            ...
            del model
            gc.collect()
            pd.DataFrame(metrics_data).to_csv(f"./results/yahoo/double_budget_tmp_{model_name}.csv")
...
df = run_experiment(models_names, models, dataset, df, calibration_column_list=['double'])
...
df.to_csv(f"./results/yahoo/double_budget_{time.time()}_complete_index1.csv")
