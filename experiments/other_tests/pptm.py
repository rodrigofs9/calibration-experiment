...
from source.rerank.rerank_algo import run_user2, run_user3, run_user_PPTM
...
# dataset.clean_dataset(users=100, movies=10)
    ...
    for ind in [int(indiceee)]:
        ...
        for model_name, model in zip(model_name_list, model_list):
            ...
            for calibration_column in calibration_column_list:
                ...
                tradeoffss =[1, 0.5, 0.1, 0.01, 0.001]
            ...
            del model
            gc.collect()
            pd.DataFrame(metrics_data).to_csv(f"./results/yahoo/PPTM_tmp_{model_name}.csv")
...
df = run_experiment(models_names, models, dataset, df, calibration_column_list=['PPTM'
])
...
df.to_csv(f"./results/yahoo/PPTM{time.time()}_complete_index1.csv")
