...
for i in range(bags):
    popularity_bins[i] = (i*interval, (i+1)*interval)
...
def run_experiment(model_name_list, model_list, dataset, df, calibration_column_list=["genres"]):
    ...
    for ind in [int(indiceee)]:
        ...
        for model_name, model in zip(model_name_list, model_list):
            ...
            del model
            gc.collect()
            pd.DataFrame(metrics_data).to_csv(f"./results/yahoo/vanilla_tmp_{model_name}.csv")
...
df = run_experiment(models_names, models, dataset, df, calibration_column_list=['nothing'
])
...
df.to_csv(f"./results/yahoo/vanilla_{time.time()}_complete_index1.csv")
