...
def run_experiment(model_name_list, model_list, dataset, df, calibration_column_list=["genres"]):
    ...
    for ind in [int(indiceee)]:
        ...
        for model_name, model in zip(model_name_list, model_list):
            ...
            del model
            gc.collect()
            pd.DataFrame(metrics_data).to_csv(f"./results/yahoo_movies/abdoullahpouri_vae_tmp_{model_name}.csv")
...
if __name__ == '__main__':
    ...
    items_path = f"./datasets/yahoo_movies/items.csv"
    ...
    ratings = pd.read_csv(f"./datasets/yahoo_movies/ratings.csv")
    ...
    if mlDataset is not None:
        ...
        df = run_experiment(models_names, models, mlDataset, df, calibration_column_list=['popularity'])
        ...
        df.to_csv(f"./results/yahoo_movies/abdoullahpouri_vae_{time.time()}_complete_index1.csv")