...
class AffinityMatrix:
    ...
    def gen_affinity_matrix(self):
        ...
        itm_id = self.df_["hashedItems"].fillna(-1).astype(int)  # itm_id serving as columns
        usr_id = self.df_["hashedUsers"].fillna(-1).astype(int)  # usr_id serving as rows
        
        valid_indices = itm_id >= 0  # Filtrar valores não negativos
        itm_id = itm_id[valid_indices]
        ratings = ratings[valid_indices]
        usr_id = usr_id[valid_indices]
