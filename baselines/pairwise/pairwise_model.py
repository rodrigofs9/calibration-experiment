...
from baselines.pairwise.pairwise_utils import save_obj, get_bpr_loss
...
class Model:
    ...
    def __init__(self, users, items, observed_relevance, unobserved_relevance, category_per_item, item_field, user_field, rating_field):
        ...
        #print(observed_relevance)
        #print(unobserved_relevance)
    ...
    def __get_representation(self):
        ...
        for user_id, user_observed_relevance in enumerate(self.observed_relevance):
            ...
            user_observed_categories = []
            for item_id in user_ubserved_items:
                if item_id in self.category_per_item:
                    category = self.category_per_item[item_id]
                    if category in self.categories:
                        category_index = self.categories.index(category)
                        categories_per_user[user_id, category_index] += 1
        ...
        for user_id, (user_observed, user_unobserved, user_predicted) in enumerate(zip(self.observed_relevance, self.unobserved_relevance, self.predicted_relevance)):
            ...
            if user_id < self.no_users:
                ...
                for index_k, k in enumerate(cutoffs):
                    ...
                    self.metrics['diversity'][index_k, user_id] = len(np.unique([self.category_per_item.get(item_id, 1) for item_id in top_k])) / self.no_categories
                    if item_group is not None:
